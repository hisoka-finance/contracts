...
contract Pair2Test is BaseTest {
    VotingEscrow escrow;
    ...
    function votingEscrowMerge() public {
        ...
        assertEq(sky.balanceOf(address(escrow)), 2 * TOKEN_1);
        ...
        assertEq(sky.balanceOf(address(escrow)), 3 * TOKEN_1);
        console2.log(escrow.totalSupply());
        escrow.merge(3, 1);
        console2.log(escrow.totalSupply());
        assertGt(escrow.balanceOfNFT(1), 1990063075414519385);
    ...
    function createPairWithNonGovernor() public {
        ...
        TestOwner(address(owner2)).approve(
            address(USDC),
            address(router),
            USDC_1
        );
        ...
        TestOwner(address(owner2)).addLiquidity(
    ...
    function mintAndBurnTokensForPairFraxUsdcOwner2() public {
        ...
        assertEq(
            owner2.getAmountOut(address(pair), USDC_1, address(USDC)),
            992220948146798746
        );
    }
    ...
    function routerPair1GetAmountsOutAndSwapExactTokensForTokensOwner2()
        public
    {
    ...
    function routerPair2GetAmountsOutAndSwapExactTokensForTokens() public {
        ...
        assertEq(
            router.getAmountsOut(USDC_1, routes)[1],
            pair2.getAmountOut(USDC_1, address(USDC))
        );
    ...
    function routerPair3GetAmountsOutAndSwapExactTokensForTokens() public {
        ...
        assertEq(
            ...
            pair3.getAmountOut(TOKEN_1M, address(FRAX))
        );
    ...
    function deployPairFactoryGauge() public {
        ...
        sky.approve(address(gaugeFactory), 15 * TOKEN_100K);
        ...
        staking = new TestStakingRewards(address(pair), address(sky));
    ...
    function withdrawGaugeStake() public {
        ...
        owner2.withdrawStake(
            address(staking),
            staking._balances(address(owner2))
        );
    ...
    function addGaugeAndBribeRewards() public {
        ...
        sky.approve(address(gauge), PAIR_1);
        sky.approve(address(xbribe), PAIR_1);
        sky.approve(address(staking), PAIR_1);

        gauge.notifyRewardAmount(address(sky), PAIR_1);
        xbribe.notifyRewardAmount(address(sky), PAIR_1);
        staking.notifyRewardAmount(PAIR_1);

        assertEq(gauge.rewardRate(address(sky)), 1653);
        // no reward rate, all or nothing
        // assertEq(xbribe.rewardRate(address(sky)), 1653);
    ...
    function createLock2() public {
        ...
        assertGt(escrow.balanceOfNFT(1), 995063075414519385);
        assertEq(sky.balanceOf(address(escrow)), 4 * TOKEN_1);
    }
    ...
    function gaugeDistributeBasedOnVoting() public {
        ...
        sky.approve(address(voter), PAIR_1);
    ...
    function routerPair1GetAmountsOutAndSwapExactTokensForTokens2Again()
        public
    {
    ...
    function routerPair2GetAmountsOutAndSwapExactTokensForTokens2Again()
        public
    {
    ...
    function minterMint() public {
        ...
        console2.log(sky.balanceOf(address(distributor)));
        console2.log(distributor.claimable(1));
        uint256 claimable = voter.claimable(address(gauge));
        sky.approve(address(staking), claimable);
    ...
    function gaugeClaimRewards() public {
        ...
        gauge.batchRewardPerToken(address(sky), 200);
        vm.warp(block.timestamp + 1);
        uint256 earned = gauge.earned(address(sky), address(owner));
        address[] memory rewards = new address[](1);
        rewards[0] = address(sky);
        vm.warp(block.timestamp + 1);
    ...
    function minterMint2() public {
        ...
        assertEq(gauge.rewardRate(address(sky)), staking.rewardRate());
        console2.log(gauge.rewardPerTokenStored(address(sky)));
    }
    ...
    function gaugeClaimRewardsOwner3NextCycle() public {
        ...
        owner3.approve(address(pair), address(gauge), PAIR_1);
        owner3.deposit(address(gauge), PAIR_1, 0);
        //uint256 before = osky.balanceOf(address(owner3));
        vm.warp(block.timestamp + 1);
        // uint256 earned = gauge.earned(address(sky), address(owner3));
        address[] memory rewards = new address[](1);
        rewards[0] = address(sky);
        owner3.getGaugeReward(address(gauge), address(owner3), rewards);
        //uint256 after_ = osky.balanceOf(address(owner3));
        //uint256 received = after_ - before;
        //assertGt(received, 0);
        console2.log(gauge.rewardPerTokenStored(address(sky)));
    ...
    function testGaugeClaimRewards3() public {
        ...
        pair.approve(address(gauge), PAIR_1);
        gauge.deposit(PAIR_1, 0);
        sky.approve(address(gauge), sky.balanceOf(address(owner)));
        gauge.notifyRewardAmount(address(sky), sky.balanceOf(address(owner)));
