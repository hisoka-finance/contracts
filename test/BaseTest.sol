...
abstract contract BaseTest is Test, TestOwner {
    ...
    uint256 internal constant TWENTY_SIX_WEEKS = 26 * ONE_WEEK;
    ...
    function deployCoins() public {
        sky = new Skydrome(msg.sender, 82800140034502500000000000);
    ...
    function mintFlow(
        ...
        for (uint256 i = 0; i < _amounts.length; i++) {
            sky.mint(_accounts[i], _amounts[i]);
        }
    }
    ...
    function deployPairWithOwner(address _owner) public {
        ...
        pair3 = Pair(address3);
        address address4 = factory.getPair(address(sky), address(DAI), false);
