...
contract Vestor is Ownable {
    ...
    /**
     * @dev Initializes the HSK token address
     *
    ...
    function claim() external returns (uint256 vestedAmount) {
        ...
        HSK.transfer(msg.sender, vestedAmount);
    }
    ...
    function vestFor(address user, uint256 amount) external onlyOwner {
        ...
        HSK.transferFrom(msg.sender, address(this), amount);
    ...
    function cancelVest(address user) external onlyOwner {
        ...
        require(
            HSK.balanceOf(address(this)) >= amount,
            "Insufficient HSK balance"
        );
        delete vest[user];
        HSK.transfer(revokeTo, amount);
