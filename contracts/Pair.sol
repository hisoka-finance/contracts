...
//@audit we can remove tank related vars and functions
// The base pair of pools, either stable or volatile

//@audit voter is hardcoded to pairs
...
contract Pair is IPair {
    ...
    function swap(
        ...
        require(amount0In > 0 || amount1In > 0, "IIA"); // Pair: INSUFFICIENT_INPUT_AMOUNT
        {
            ...
            if (amount0In > 0)
                _sendTokenFees(
                    token0,
                    (amount0In * IPairFactory(factory).getFee(address(this))) /
                        10000
                );
            if (amount1In > 0)
                _sendTokenFees(
                    token1,
                    (amount1In * IPairFactory(factory).getFee(address(this))) /
                        10000
                );
    ...
    function getAmountOut(
        ...
        (uint _reserve0, uint _reserve1) = (reserve0, reserve1);
        amountIn -=
            (amountIn * IPairFactory(factory).getFee(address(this))) /
